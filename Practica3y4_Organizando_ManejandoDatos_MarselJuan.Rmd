...
```{r}
pew_religion.t <- gather(pew_religion,income,freq,-religion,factor_key = TRUE)
head(pew_religion.t)
```
...
```{r}
raw.t <- pivot_longer(raw, cols = contains('wk'), names_to='week', values_to='rank',names_prefix = 'wk')
head(raw.t)

# Poniendo columna con la fecha
library(lubridate)
raw.t_date <- raw.t[,!colnames(raw.t)=='week']
raw.t_date$date.entered <- as.Date(raw.t_date$date.entered)
raw.t_date$date <- raw.t_date$date.entered + weeks(raw.t$week)
# ordenar
raw.t_date[,c(6,7)] <- raw.t_date[,c(7,6)]
head(raw.t_date)
```
...
```{r}
dtb <- raw %>% gather(key=grupo, value=casos,-country,-year) %>% separate(col=grupo,into=c('sexo','edad'), sep=1)
# sep=1 porque quieres que separe la primera letra de lo demás
# Formato mejor para los rangos de edad
dtb$edad <- paste0(substr(dtb$edad, nchar(dtb$edad)-3, nchar(dtb$edad)-2),'-',substr(dtb$edad, nchar(dtb$edad)-1, nchar(dtb$edad)))
head(dtb)
```
...
```{r}
weather.t <- raw %>% pivot_longer(cols = starts_with('d'),names_to = 'día', values_to = 'Temperatura', names_prefix = 'd', values_drop_na = TRUE) %>% pivot_wider(names_from = 'element', values_from='Temperatura')
...
head(flights,10)
head(planes,10)
# Se pueden unir a partir de la variable tailnum
```
...
```{r }
library(stringr)
#planes.f <- planes %>% filter(by=(grepl('DOUGLAS',planes$manufacturer,ignore.case = T))&(seats>=90)&(seats<=210))

planes.f <- planes %>% filter(!str_detect(str_to_lower(manufacturer),'.*DOUGLAS.*')) %>% filter((seats>=90)&(seats<=210)) %>% mutate(manufacturer=ifelse(str_detect(str_to_lower(manufacturer),'.*airbus.*'),'AIRBUS COMP',manufacturer))
#* número de veces que sea, a diferencia de + también puede haber 0 (en vez de 1 o más para+)
# filter(between(seats,0,90))
```
...
```{r }
fligths.f <- flights %>%
  inner_join(planes.f %>% select(tailnum), by='tailnum') # Tabla flights solo para los que están en planes, seleccionamos solo planes.f tailnum para que no añada las columnas de planes.f
...
```{r }
# Se puede eliminar con planes$speed <- null
...
```{r }
flights <- flights %>%
  unite('Full_Date',c(day,month,year),sep='-',remove = TRUE) # fusionar columnas
